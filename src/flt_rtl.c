...
/**
 *  Take the floatValue from the parameters and call fltCmp().
 ...
 *          second.
 */
intType fltCmpValue (const rtlValueUnion value1, const rtlValueUnion value2)

  { /* fltCmpValue */
    return fltCmp(value1.floatValue, value2.floatValue);
  } /* fltCmpValue */



/**
 *  Take the floatValue from the parameters and assign source to dest.
 *  Function pointers in C programs generated by the Seed7 compiler
 *  may point to this function.
 */
void fltCpyValue (rtlValueUnion *const dest, const rtlValueUnion source)
...
striType fltStr (floatType number)
  ...
  /* fltStr */
    ...
    if (os_isnan(number)) {
      ...
      buffer_ptr = buffer;
      len = doubleToCharBuffer(number, FLOATTYPE_STR_LARGE_NUMBER,
                               FMT_E, buffer);
    } /* if */
